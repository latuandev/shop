        ...
        <section class="content">
            <div class="container-fluid">
                <div class="card">
                    <div class="card-header" style="display: flex; align-items: center;">
                        ...
                        <a href="{{ route('admin.add-view.products') }}" class="btn btn-primary">Thêm</a>
                    </div>
                    <div class="card-body">
            ...
            <script>
                ...
                function changeFunction(id, status) {
                    $.ajax({
                        type: "post",
                        url: "{{route('admin.change-status.products')}}",
                        data: {
                            id: id,
                            status: status
                        },
                ...
                function deleteFunction(url) {
                    bootbox.confirm("Bạn có chắc chắn muốn sản phẩm này?", function(result) {
                        if (result == true) {
                            $.ajax({
                                type: "post",
                                url: "{{route('admin.delete.products')}}",
                                data: {
                                    url: url
                                },
                                dataType: "json",
                                success: function(response) {
                                    if (response.pass) {
                                        toastr.success(response.pass)
                                        $('#tbl-products').DataTable().ajax.reload(null,
